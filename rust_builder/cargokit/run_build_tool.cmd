...
if not defined DART (
    for /f "usebackq delims=" %%I in (`where.exe dart 2^>nul`) do (
        SET DART=%%I
        goto :dart_found
    )
)
...
if not defined DART (
    for /f "usebackq delims=" %%I in (`where.exe flutter 2^>nul`) do (
        SET DART=%%~dpIcache\dart-sdk\bin\dart.exe
        goto :dart_found
    )
)
...
:dart_found
if not defined DART (
    echo Unable to find Dart SDK. Set FLUTTER_ROOT or add dart/flutter to PATH.
    exit /b 1
)
